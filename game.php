...
if ($_GET["game"]) { $game = $_GET["game"]; }
if ($_GET["game_id"]) { $game = $_GET["game_id"]; }
if ($_GET["week"]) { $week = $_GET["week"]; }
if ($_GET["year"]) { $year = $_GET["year"]; }
if ($_GET["type"]) { $season_type = $_GET["type"]; }
if ($_GET["data"]) { $data = $_GET["data"]; }
if (!isset($game)) { header('Location: /404.html'); die(); }
...
// NAV FUNCTIONS - GAMES
$grab_games = CACHE . 'game/game?season_type=eq.'.$season_type.'&season_year=eq.'.$year.'&week=eq.'.$week.'&select=gsis_id,home_team,away_team&order=start_time.asc.json';
if (file_exists($grab_games) && filemtime($grab_games) + 604800 > time()) {
	$grabgamesdata = file_get_contents($grab_games);
	$grabgamesobj = json_decode($grabgamesdata, true);
} else {
	$msg = file_get_contents($postgrest . 'game?season_type=eq.'.$season_type.'&season_year=eq.'.$year.'&week=eq.'.$week.'&select=gsis_id,home_team,away_team&order=start_time.asc');
	$f = fopen($grab_games, "w+");
	fwrite($f, $msg);
	fclose($f);
	$grabgamesobj = json_decode($msg, true);
}
$i = 0;
foreach ($grabgamesobj as $key => $val) {
	if ($game == $val["gsis_id"]) {
		$totalgames[$i] = '<li class="active"><a href="game.php?game_id='.$val["gsis_id"].'">'.$val["away_team"].' vs. '.$val["home_team"].'</a></li>';
	} else {
		$totalgames[$i] = '<li><a href="game.php?game_id='.$val["gsis_id"].'">'.$val["away_team"].' vs. '.$val["home_team"].'</a></li>';
	}
	$i++;
}
$i = 0;
...
foreach ($objseasonscore as $key => $val) {
	$finishedgames["".$val["home_team"].""] = $val["gsis_id"];
	$finishedgames["".$val["away_team"].""] = $val["gsis_id"];
	$ht = $val["home_team"];
	$at = $val["away_team"];
	$hts = $val["home_score"];
	$ats = $val["away_score"];
...
$i = 0;
$json_string = 'http://localhost:2096/game?gsis_id=eq.'.$game;
...
foreach ($obj as $key => $val) {
	...
	$hometeam_string = 'http://localhost:2096/team?team_id=eq.'.$homecode;
	$awayteam_string = 'http://localhost:2096/team?team_id=eq.'.$awaycode;
	...
	$drive_string = 'http://localhost:2096/drive?gsis_id=eq.'.$date.'&order=time_inserted.desc';
	...
	if ($quarter !== "P") {
		...
		if ($quarter !== "F") {
			...
			$bottom_line = '<div class="alert fade in" style="margin-bottom: 0px;border-radius: 0px;">
			...
			foreach ($objdrive as $key => $drives) {
				...
				} else if ($drives["pos_team"] === $homecode) {
					$drives_stats .= '<tr>
					<td>'.$drives["drive_id"].'</td>
					<td>'.$drives["end_time"]["phase"].'</td>
					<td>'.$drives["pos_team"].'</td>
			...
			$playbyplay_string = 'http://localhost:2096/play?gsis_id=eq.'.$date.'&order=drive_id.desc,play_id.desc';
			...
			$tabbody = '<div class="col-md-6" style="padding:0px;">
							'.$drives_table.'
						</div>
			...
			$progress_button = '<span class="label label-success m-r-10 pull-right"><strong>FINAL</strong></span>';
			/*$bottom_line = '<div class="alert fade in" style="margin-bottom: 0px;border-radius: 0px;">
							<ul class="list-inline">
								<li><strong style="color:#151515;">Summary Stats</strong></li>
							</ul>
						</div>';*/
			$bottom_line = '';
			...
			foreach ($objdrive as $key => $drives) {
				if ($drives["pos_team"] === $awaycode) {
					...
					$agg_play_string = 'http://localhost:2096/agg_play?gsis_id=eq.'.$date.'&drive_id=eq.'.$drives["drive_id"].'&rushing_att=eq.1&select=rushing_yds';
					...
					$agg_play_string2 = 'http://localhost:2096/agg_play?gsis_id=eq.'.$date.'&drive_id=eq.'.$drives["drive_id"].'&passing_att=eq.1&select=passing_yds';
					...
					$agg_play_string3 = 'http://localhost:2096/agg_play?gsis_id=eq.'.$date.'&drive_id=eq.'.$drives["drive_id"].'&select=passing_sk_yds';
			...
			$playbyplay_string = 'http://localhost:2096/play?gsis_id=eq.'.$date.'&order=drive_id.asc,play_id.asc';
			$jsonplaybyplay = file_get_contents($playbyplay_string);
			$objplaybyplay = json_decode($jsonplaybyplay,true);
			...
			$passing_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=team,player_id,passing_att,passing_cmp,passing_cmp_air_yds,passing_incmp,passing_incmp_air_yds,passing_int,passing_sk,passing_sk_yds,passing_tds,passing_twopta,passing_twoptm,passing_twoptmissed,passing_yds&passing_att=not.eq.0&order=team.asc,player_id.desc';
			...
			$rushing_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&select=team,player_id,rushing_att,rushing_loss,rushing_loss_yds,rushing_tds,rushing_twopta,rushing_twoptm,rushing_twoptmissed,rushing_yds&order=team.asc,player_id.desc';
			...
			foreach ($rushing_id_away as $rush => $rushstat) {
			    ...
			    $rushing_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&player_id=eq.'.$rushstat.'&select=team,player_id,rushing_att,rushing_loss,rushing_loss_yds,rushing_tds,rushing_twopta,rushing_twoptm,rushing_twoptmissed,rushing_yds&rushing_att=not.eq.0&order=team.asc,player_id.desc';
			    $jsonrushingaway = file_get_contents($rushing_string_away);
			    $objrushingaway = json_decode($jsonrushingaway,true);
			    $rushing_att_away = 0;
				$rushing_yds_away = 0;
			...
			$rushing_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=team,player_id,rushing_att,rushing_loss,rushing_loss_yds,rushing_tds,rushing_twopta,rushing_twoptm,rushing_twoptmissed,rushing_yds&rushing_att=not.eq.0&order=team.asc,player_id.desc';
			...
			foreach ($rushing_id_home as $rush => $rushstat) {
			    ...
			    $rushing_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&player_id=eq.'.$rushstat.'&select=team,player_id,rushing_att,rushing_loss,rushing_loss_yds,rushing_tds,rushing_twopta,rushing_twoptm,rushing_twoptmissed,rushing_yds&rushing_att=not.eq.0&order=team.asc,player_id.desc';
			    $jsonrushinghome = file_get_contents($rushing_string_home);
			    $objrushinghome = json_decode($jsonrushinghome,true);
			    $rushing_att_home = 0;
				$rushing_yds_home = 0;
			...
			$receiving_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&select=team,player_id,receiving_rec,receiving_tar,receiving_tds,receiving_twopta,receiving_twoptm,receiving_twoptmissed,receiving_yac_yds,receiving_yds&receiving_rec=not.eq.0&order=team.asc,player_id.desc';
			...
			foreach ($receiving_id_away as $receive => $receivestat) {
				...
				$objplayeraway = json_decode($jsonplayeraway,true);
				$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
			    $receiving_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&player_id=eq.'.$receivestat.'&select=team,player_id,receiving_rec,receiving_tar,receiving_tds,receiving_twopta,receiving_twoptm,receiving_twoptmissed,receiving_yac_yds,receiving_yds&order=team.asc,player_id.desc';
			    $jsonreceivingaway = file_get_contents($receiving_string_away);
			    $objreceivingaway = json_decode($jsonreceivingaway,true);
			    $receiving_rec_away = 0;
				$receiving_yds_away = 0;
			...
			$receiving_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=team,player_id,receiving_rec,receiving_tar,receiving_tds,receiving_twopta,receiving_twoptm,receiving_twoptmissed,receiving_yac_yds,receiving_yds&receiving_rec=not.eq.0&order=team.asc,player_id.desc';
			...
			foreach ($receiving_id_home as $receive => $receivestat) {
				...
				$objplayerhome = json_decode($jsonplayerhome,true);
				$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
			    $receiving_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&player_id=eq.'.$receivestat.'&select=team,player_id,receiving_rec,receiving_tar,receiving_tds,receiving_twopta,receiving_twoptm,receiving_twoptmissed,receiving_yac_yds,receiving_yds&order=team.asc,player_id.desc';
			    $jsonreceivinghome = file_get_contents($receiving_string_home);
			    $objreceivinghome = json_decode($jsonreceivinghome,true);
			    $receiving_rec_home = 0;
				$receiving_yds_home = 0;
			...
			$fumble_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&select=player_id';
			...
			foreach ($fumble_id_away as $fumble => $fumblestat) {
				...
				$objplayeraway = json_decode($jsonplayeraway,true);
				$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
				$fumble_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&player_id=eq.'.$fumblestat.'&select=fumbles_lost,fumbles_rec,fumbles_rec_tds,fumbles_tot,fumbles_rec_yds,defense_frec,defense_frec_yds';
				$jsonfumbleaway = file_get_contents($fumble_string_away);
				$objfumbleaway = json_decode($jsonfumbleaway,true);
				$fumbles_lost_away = 0;
				$fumbles_rec_away = 0;
			...
			$fumble_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=player_id';
			...
			foreach ($fumble_id_home as $fumble => $fumblestat) {
				...
				$objplayerhome = json_decode($jsonplayerhome,true);
				$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
				$fumble_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&player_id=eq.'.$fumblestat.'&select=fumbles_lost,fumbles_rec,fumbles_rec_tds,fumbles_tot,fumbles_rec_yds,defense_frec,defense_frec_yds';
				$jsonfumblehome = file_get_contents($fumble_string_home);
				$objfumblehome = json_decode($jsonfumblehome,true);
				$fumbles_lost_home = 0;
				$fumbles_rec_home = 0;
			...
			$kicking_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&select=team,player_id,kicking_fga,kicking_fgm,kicking_xpa,kicking_xpmade,kicking_yds&order=team.asc,player_id.desc';
			...
			foreach ($kicking_id_away as $kicking => $kickingstat) {
				...
				$objplayeraway = json_decode($jsonplayeraway,true);
				$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
				$kicking_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&player_id=eq.'.$kickingstat.'&select=team,player_id,kicking_fga,kicking_fgm,kicking_xpa,kicking_xpmade,kicking_yds&order=team.asc,player_id.desc';
				$jsonkickingaway = file_get_contents($kicking_string_away);
				$objkickingaway = json_decode($jsonkickingaway,true);
				$kicking_fga_away = 0;
				$kicking_fgm_away = 0;
			...
			$kicking_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=team,player_id,kicking_fga,kicking_fgm,kicking_xpa,kicking_xpmade,kicking_yds&order=team.asc,player_id.desc';
			...
			foreach ($kicking_id_home as $kicking => $kickingstat) {
				...
				$objplayerhome = json_decode($jsonplayerhome,true);
				$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
				$kicking_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&player_id=eq.'.$kickingstat.'&select=team,player_id,kicking_fga,kicking_fgm,kicking_xpa,kicking_xpmade,kicking_yds&order=team.asc,player_id.desc';
				$jsonkickinghome = file_get_contents($kicking_string_home);
				$objkickinghome = json_decode($jsonkickinghome,true);
				$kicking_fga_home = 0;
				$kicking_fgm_home = 0;
			...
			$punting_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&select=team,player_id,punting_tot,punting_yds,punting_i20&order=team.asc,player_id.desc';
			...
			foreach ($punting_id_away as $punting => $puntingstat) {
				...
				$objplayeraway = json_decode($jsonplayeraway,true);
				$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
				$punting_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&player_id=eq.'.$puntingstat.'&select=team,player_id,punting_tot,punting_yds,punting_i20&order=team.asc,player_id.desc';
				$jsonpuntingaway = file_get_contents($punting_string_away);
				$objpuntingaway = json_decode($jsonpuntingaway,true);
				$punting_tot_away = 0;
				$punting_yds_away = 0;
			...
			$punting_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=team,player_id,punting_tot,punting_yds,punting_i20&order=team.asc,player_id.desc';
			...
			foreach ($punting_id_home as $punting => $puntingstat) {
				...
				$objplayerhome = json_decode($jsonplayerhome,true);
				$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
				$punting_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&player_id=eq.'.$puntingstat.'&select=team,player_id,punting_tot,punting_yds,punting_i20&order=team.asc,player_id.desc';
				$jsonpuntinghome = file_get_contents($punting_string_home);
				$objpuntinghome = json_decode($jsonpuntinghome,true);
				$punting_tot_home = 0;
				$punting_yds_home = 0;
			...
			$kickret_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&select=team,player_id,kickret_fair,kickret_oob,kickret_ret,kickret_tds,kickret_touchback,kickret_yds&order=team.asc,player_id.desc';
			...
			foreach ($kickret_id_away as $kickret => $kickretstat) {
				...
				$objplayeraway = json_decode($jsonplayeraway,true);
				$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
				$kickret_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&player_id=eq.'.$kickretstat.'&select=team,player_id,kickret_fair,kickret_oob,kickret_ret,kickret_tds,kickret_touchback,kickret_yds&order=team.asc,player_id.desc';
				$jsonkickretaway = file_get_contents($kickret_string_away);
				$objkickretaway = json_decode($jsonkickretaway,true);
				$kickret_ret_away = 0;
				$kickret_tds_away = 0;
			...
			$kickret_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=team,player_id,kickret_fair,kickret_oob,kickret_ret,kickret_tds,kickret_touchback,kickret_yds&order=team.asc,player_id.desc';
			...
			foreach ($kickret_id_home as $kickret => $kickretstat) {
				...
				$objplayerhome = json_decode($jsonplayerhome,true);
				$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
				$kickret_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&player_id=eq.'.$kickretstat.'&select=team,player_id,kickret_fair,kickret_oob,kickret_ret,kickret_tds,kickret_touchback,kickret_yds&order=team.asc,player_id.desc';
				$jsonkickrethome = file_get_contents($kickret_string_home);
				$objkickrethome = json_decode($jsonkickrethome,true);
				$kickret_ret_home = 0;
				$kickret_tds_home = 0;
			...
			$puntret_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&select=team,player_id,puntret_fair,puntret_oob,puntret_tot,puntret_tds,puntret_touchback,puntret_yds&order=team.asc,player_id.desc';
			...
			foreach ($puntret_id_away as $puntret => $puntretstat) {
				...
				$objplayeraway = json_decode($jsonplayeraway,true);
				$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
				$puntret_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&player_id=eq.'.$puntretstat.'&select=team,player_id,puntret_fair,puntret_oob,puntret_tot,puntret_tds,puntret_touchback,puntret_yds&order=team.asc,player_id.desc';
				$jsonpuntretaway = file_get_contents($puntret_string_away);
				$objpuntretaway = json_decode($jsonpuntretaway,true);
				$puntret_tot_away = 0;
				$puntret_tds_away = 0;
			...
			$puntret_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=team,player_id,puntret_fair,puntret_oob,puntret_tot,puntret_tds,puntret_touchback,puntret_yds&order=team.asc,player_id.desc';
			...
			foreach ($puntret_id_home as $puntret => $puntretstat) {
				...
				$objplayerhome = json_decode($jsonplayerhome,true);
				$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
				$puntret_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&player_id=eq.'.$puntretstat.'&select=team,player_id,puntret_fair,puntret_oob,puntret_tot,puntret_tds,puntret_touchback,puntret_yds&order=team.asc,player_id.desc';
				$jsonpuntrethome = file_get_contents($puntret_string_home);
				$objpuntrethome = json_decode($jsonpuntrethome,true);
				$puntret_tot_home = 0;
				$puntret_tds_home = 0;
			...
			$defense_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&select=team,player_id,defense_ast,defense_tkl,defense_sk,defense_int,defense_ffum&order=defense_tkl.desc,player_id.desc';
			...
			foreach ($defense_id_away as $defense => $defensestat) {
				...
				$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
				$defense_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$awaycode.'&player_id=eq.'.$defensestat.'&select=team,player_id,defense_ast,defense_tkl,defense_sk,defense_int,defense_ffum,defense_int_tds,defense_frec_tds&order=defense_tkl.desc,player_id.desc';
				$jsondefenseaway = file_get_contents($defense_string_away);
				$objdefenseaway = json_decode($jsondefenseaway,true);
				$defense_ast_away = 0;
				$defense_tkl_away = 0;
			...
			$defense_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&select=team,player_id,defense_ast,defense_tkl,defense_sk,defense_int,defense_int_tds,defense_ffum&order=defense_tkl.desc,player_id.desc';
			...
			foreach ($defense_id_home as $defense => $defensestat) {
				...
				$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
				$defense_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$date.'&team=eq.'.$homecode.'&player_id=eq.'.$defensestat.'&select=team,player_id,defense_ast,defense_tkl,defense_sk,defense_int,defense_ffum,defense_int_tds,defense_frec_tds&order=defense_tkl.desc,player_id.desc';
				$jsondefensehome = file_get_contents($defense_string_home);
				$objdefensehome = json_decode($jsondefensehome,true);
				$defense_ast_home = 0;
				$defense_tkl_home = 0;
			...
			//arsort($globalleader_passing_yds);
			//arsort($globalleader_rushing_yds);
			//arsort($globalleader_receiving_yds);

			$summary_table_away = '<table class="table" style="margin-bottom:0px;">
								<thead>
									<tr class="'.$awaycode.'colors">
										<th>Team Leaders: <a href="team.php?team='.$awaycode.'">'.$awayteam.'</a> <span style="font-weight:normal;">('.$wins["".$awaycode.""].'-'.$losses["".$awaycode.""].'-'.$ties["".$awaycode.""].')</span></th>
										<th></th>
										<th></th>
										<th></th>
										<th></th>
			...
			$summary_table_home = '<table class="table" style="margin-bottom:0px;">
								<thead>
									<tr class="'.$homecode.'colors">
										<th>Team Leaders: <a href="team.php?team='.$homecode.'">'.$hometeam.'</a> <span style="font-weight:normal;">('.$wins["".$homecode.""].'-'.$losses["".$homecode.""].'-'.$ties["".$homecode.""].')</span></th>
										<th></th>
										<th></th>
										<th></th>
										<th></th>
			...
			$stats_table_away_faux = '<table class="table table-bordered display3" style="margin-bottom:0px;z-index: -1;position: absolute;">
								<thead>
									<tr class="'.$awaycode.'colors">
										<th><a href="team.php?team='.$awaycode.'" class="'.$awaycode.'colors">'.$awayteam.'</a> <span style="font-weight:normal;" class="'.$awaycode.'colors">('.$wins["".$awaycode.""].'-'.$losses["".$awaycode.""].'-'.$ties["".$awaycode.""].')</span></th>
										<th></th>
										<th></th>
										<th></th>
										<th></th>
			...
			$stats_table_home_faux = '<table class="table table-bordered display3" style="margin-bottom:0px;z-index: -1;position: absolute;">
								<thead>
									<tr class="'.$homecode.'colors">
										<th><a href="team.php?team='.$homecode.'" class="'.$homecode.'colors">'.$hometeam.'</a> <span style="font-weight:normal;" class="'.$homecode.'colors">('.$wins["".$homecode.""].'-'.$losses["".$homecode.""].'-'.$ties["".$homecode.""].')</span></th>
										<th></th>
										<th></th>
										<th></th>
										<th></th>
			...
			$front_summary_away_string = 'http://localhost:2096/play?gsis_id=eq.'.$date.'&pos_team=eq.'.$awaycode.'&order=drive_id.asc,play_id.asc';
			...
			$front_summary_home_string = 'http://localhost:2096/play?gsis_id=eq.'.$date.'&pos_team=eq.'.$homecode.'&order=drive_id.asc,play_id.asc';
			...
			<table class="table" style="margin-bottom:0px;">
								<thead>
									<tr class="'.$homecode.'colors">
										<th class="border-color: transparent!important;"><a href="team.php?team='.$homecode.'" class="'.$homecode.'colors">'.$hometeam.'</a> <span class="'.$homecode.'colors" style="font-weight:normal;">('.$wins["".$homecode.""].'-'.$losses["".$homecode.""].'-'.$ties["".$homecode.""].')</span></th>
			...
			$tabs = '<li class="active"><a href="#'.$hashtag.'-tab-1" data-toggle="tab" aria-expanded="true">Summary</a></li>
					...
					<li class=""><a href="#'.$hashtag.'-tab-4" data-toggle="tab" aria-expanded="false">Drives</a></li>';
			$tabbody = '<div class="tab-pane fade" id="'.$hashtag.'-tab-2">
						...
						<div class="tab-pane fade" id="'.$hashtag.'-tab-3">
							<div data-scrollbar="true" data-height="auto">
								'.$playbyplay_table.'
							</div>
						</div>
						<div class="tab-pane fade" id="'.$hashtag.'-tab-4">
							<div data-scrollbar="true" data-height="auto">
								'.$drives_table.'
							</div>
						</div>';
			...
			//if ($data === "visual") {
				...
				if ($homescore > $awayscore) {
					$adjusted = $homescore;
					$winbg = "background: url('/assets/img/teams/".$homecode."_lg.png') no-repeat;background-size: cover;background-position-y: 50%;background-color: #000;";
					...
					$adjusted = $awayscore;
					$winbg = "background: url('/assets/img/teams/".$awaycode."_lg.png') no-repeat;background-size: cover;background-position-y: 50%;background-color: #000;";
					$winbg2 = "-ms-filter:'progid:DXImageTransform.Microsoft.Alpha(Opacity=99)';filter: alpha(opacity=99);opacity: 0.99;background-color:#222!important;";
				} else {
					$winbg = '';
					$winbg2 = '';
				...
				$score_morris_area_chart = '<div class="col-md-12" style="padding:0px;'.$winbg.'">
			        <div class="widget-chart with-sidebar bg-black" style="-webkit-border-radius: 0px;-moz-border-radius: 0px;border-radius: 0px;margin-bottom: -35px;padding-bottom: 15px;'.$winbg2.'">
			            <div class="widget-chart-content">
			                <h4 class="chart-title">
			                    Score Results
			                    <small>per quarter including OT</small>
			                </h4>
			            ...
			            <div class="widget-chart-sidebar bg-black-darker" style="background-color:#151515!important;">
			                <div class="chart-number">
			                    Yards Gained
			                    <small>net yards gained</small>
			                </div>
			                <div id="score-donut-chart" style="height: 160px"></div>
			                <ul class="chart-legend">
			                    <li><i class="fa fa-circle-o fa-fw '.$awaycode.'font m-r-5"></i> '.$total_away_net_yards_gained.' <span>'.$awayteam.'</span></li>
			                    <li><i class="fa fa-circle-o fa-fw '.$homecode.'font m-r-5"></i> '.$total_home_net_yards_gained.' <span>'.$hometeam.'</span></li>
			                </ul>
			            </div>
			        </div>
			    </div>';
...
var handleScoreLineChart = function() {
    var away = '".$teamcolor["".$awaycode.""]."';
    var awayLight = '".$teamcolor["".$awaycode.""]."';
    var home = '".$teamcolor["".$homecode.""]."';
    var homeLight = '".$teamcolor["".$homecode.""]."';
    ...
    Morris.Line({
        ...
        data: [
            {x: 'Q1', y: ".$awayscore_q1.", z: ".$homescore_q1."},
            {x: 'Q2', y: ".$q2a.", z: ".$q2h."},
            {x: 'Q3', y: ".$q3a.", z: ".$q3h."},
            {x: 'Q4', y: ".$q4a.", z: ".$q4h."},
            {x: 'OT', y: ".$q5a.", z: ".$q5h."},
            {x: 'Final', y: ".$awayscore.", z: ".$homescore."}
        ],
        xkey: 'x',
        ykeys: ['y', 'z'],
        parseTime: false,
        labels: ['".$awayteam."', '".$hometeam."'],
        lineColors: [away, home],
        pointFillColors: [awayLight, homeLight],
        lineWidth: '2px',
...
var handleNetYdsDonutChart = function() {
    ...
    Morris.Donut({
        ...
        data: [
            {label: \"".$awayteam."\", value: ".$total_away_net_yards_gained."},
            {label: \"".$hometeam."\", value: ".$total_home_net_yards_gained."}
        ],
        colors: [away, home],
...
var handleFdownsYdsDonutChart = function() {
    ...
    Morris.Donut({
        ...
        data: [
            {label: \"".$awayteam."\", value: ".$firstdowns_away_total."},
            {label: \"".$hometeam."\", value: ".$firstdowns_home_total."}
        ],
        colors: [away, home],
        labelFamily: 'Open Sans',
        labelColor: 'rgba(255,255,255,0.4)',
...
var handleThirddownsYdsDonutChart = function() {
    var away = '".$teamcolor["".$awaycode.""]."';
    var awayLight = '".$teamcolor["".$awaycode.""]."';
    var home = '".$teamcolor["".$homecode.""]."';
    var homeLight = '".$teamcolor["".$homecode.""]."';
    Morris.Donut({
        element: 'thirddowns-donut-chart',
        data: [
            {label: \"".$awayteam."\", value: ".substr($third_down_eff_away, 0, -1)."},
            {label: \"".$hometeam."\", value: ".substr($third_down_eff_home, 0, -1)."}
        ],
        colors: [away, home],
        labelFamily: 'Open Sans',
        labelColor: 'rgba(255,255,255,0.4)',
...
DashboardV2.init();";
		...
		$grabseasonscore = 'http://localhost:2096/game?season_type=eq.'.$season_type.'&season_year=eq.'.$lastyear.'&home_team=eq.'.$homecode.'&away_team=eq.'.$awaycode.'&finished=eq.true';
		$jsonseasonscore = file_get_contents($grabseasonscore);
		$objseasonscore = json_decode($jsonseasonscore,true);

		foreach ($objseasonscore as $key => $val) {

			$pastgame = $val["gsis_id"];
			$homescore = $val["home_score"];
			$awayscore = $val["away_score"];
			// HOME SCORE QUARTER
			$homescore_q1 = $val["home_score_q1"];
		...
		if ($pastgame == '') {
			...
			$grabseasonscore = 'http://localhost:2096/game?season_type=eq.'.$season_type.'&season_year=eq.'.$lastyear.'&home_team=eq.'.$awaycode.'&away_team=eq.'.$homecode.'&finished=eq.true';
			$jsonseasonscore = file_get_contents($grabseasonscore);
			$objseasonscore = json_decode($jsonseasonscore,true);
		...
		if ($pastgame !== '') {
			...
			$date_p = $pastgame."0000";
			$date_p = strtotime($date_p);
			$date_p = date('D\, M j', $date_p);
				...
				$passing_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=team,player_id,passing_att,passing_cmp,passing_cmp_air_yds,passing_incmp,passing_incmp_air_yds,passing_int,passing_sk,passing_sk_yds,passing_tds,passing_twopta,passing_twoptm,passing_twoptmissed,passing_yds&passing_att=not.eq.0&order=team.asc,player_id.desc';
				$jsonpassingaway = file_get_contents($passing_string_away);
				$objpassingaway = json_decode($jsonpassingaway,true);
				...
				$passing_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=team,player_id,passing_att,passing_cmp,passing_cmp_air_yds,passing_incmp,passing_incmp_air_yds,passing_int,passing_sk,passing_sk_yds,passing_tds,passing_twopta,passing_twoptm,passing_twoptmissed,passing_yds&passing_att=not.eq.0&order=team.asc,player_id.desc';
				$jsonpassinghome = file_get_contents($passing_string_home);
				$objpassinghome = json_decode($jsonpassinghome,true);
				...
				$rushing_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=team,player_id,rushing_att,rushing_loss,rushing_loss_yds,rushing_tds,rushing_twopta,rushing_twoptm,rushing_twoptmissed,rushing_yds&order=team.asc,player_id.desc';
				$jsonrushingaway = file_get_contents($rushing_string_away);
				$objrushingaway = json_decode($jsonrushingaway,true);
				...
				foreach ($rushing_id_away as $rush => $rushstat) {
					...
					$objplayeraway = json_decode($jsonplayeraway,true);
					$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
				    $rushing_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&player_id=eq.'.$rushstat.'&select=team,player_id,rushing_att,rushing_loss,rushing_loss_yds,rushing_tds,rushing_twopta,rushing_twoptm,rushing_twoptmissed,rushing_yds&rushing_att=not.eq.0&order=team.asc,player_id.desc';
				    $jsonrushingaway = file_get_contents($rushing_string_away);
				    $objrushingaway = json_decode($jsonrushingaway,true);
				    $rushing_att_away = 0;
					$rushing_yds_away = 0;
				...
				$rushing_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=team,player_id,rushing_att,rushing_loss,rushing_loss_yds,rushing_tds,rushing_twopta,rushing_twoptm,rushing_twoptmissed,rushing_yds&rushing_att=not.eq.0&order=team.asc,player_id.desc';
				$jsonrushinghome = file_get_contents($rushing_string_home);
				$objrushinghome = json_decode($jsonrushinghome,true);
				...
				foreach ($rushing_id_home as $rush => $rushstat) {
					...
					$objplayerhome = json_decode($jsonplayerhome,true);
					$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
				    $rushing_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&player_id=eq.'.$rushstat.'&select=team,player_id,rushing_att,rushing_loss,rushing_loss_yds,rushing_tds,rushing_twopta,rushing_twoptm,rushing_twoptmissed,rushing_yds&rushing_att=not.eq.0&order=team.asc,player_id.desc';
				    $jsonrushinghome = file_get_contents($rushing_string_home);
				    $objrushinghome = json_decode($jsonrushinghome,true);
				    $rushing_att_home = 0;
					$rushing_yds_home = 0;
				...
				$receiving_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=team,player_id,receiving_rec,receiving_tar,receiving_tds,receiving_twopta,receiving_twoptm,receiving_twoptmissed,receiving_yac_yds,receiving_yds&receiving_rec=not.eq.0&order=team.asc,player_id.desc';
				$jsonreceivingaway = file_get_contents($receiving_string_away);
				$objreceivingaway = json_decode($jsonreceivingaway,true);
				...
				foreach ($receiving_id_away as $receive => $receivestat) {
					...
					$objplayeraway = json_decode($jsonplayeraway,true);
					$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
				    $receiving_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&player_id=eq.'.$receivestat.'&select=team,player_id,receiving_rec,receiving_tar,receiving_tds,receiving_twopta,receiving_twoptm,receiving_twoptmissed,receiving_yac_yds,receiving_yds&order=team.asc,player_id.desc';
				    $jsonreceivingaway = file_get_contents($receiving_string_away);
				    $objreceivingaway = json_decode($jsonreceivingaway,true);
				    $receiving_rec_away = 0;
					$receiving_yds_away = 0;
				...
				$receiving_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=team,player_id,receiving_rec,receiving_tar,receiving_tds,receiving_twopta,receiving_twoptm,receiving_twoptmissed,receiving_yac_yds,receiving_yds&receiving_rec=not.eq.0&order=team.asc,player_id.desc';
				$jsonreceivinghome = file_get_contents($receiving_string_home);
				$objreceivinghome = json_decode($jsonreceivinghome,true);
				...
				foreach ($receiving_id_home as $receive => $receivestat) {
					...
					$objplayerhome = json_decode($jsonplayerhome,true);
					$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
				    $receiving_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&player_id=eq.'.$receivestat.'&select=team,player_id,receiving_rec,receiving_tar,receiving_tds,receiving_twopta,receiving_twoptm,receiving_twoptmissed,receiving_yac_yds,receiving_yds&order=team.asc,player_id.desc';
				    $jsonreceivinghome = file_get_contents($receiving_string_home);
				    $objreceivinghome = json_decode($jsonreceivinghome,true);
				    $receiving_rec_home = 0;
					$receiving_yds_home = 0;
				...
				$fumble_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=player_id';
				$jsonfumbleaway = file_get_contents($fumble_string_away);
				$objfumbleaway = json_decode($jsonfumbleaway,true);
				...
				foreach ($fumble_id_away as $fumble => $fumblestat) {
					...
					$objplayeraway = json_decode($jsonplayeraway,true);
					$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
					$fumble_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&player_id=eq.'.$fumblestat.'&select=fumbles_lost,fumbles_rec,fumbles_rec_tds,fumbles_tot,fumbles_rec_yds,defense_frec,defense_frec_yds';
					$jsonfumbleaway = file_get_contents($fumble_string_away);
					$objfumbleaway = json_decode($jsonfumbleaway,true);
					$fumbles_lost_away = 0;
					$fumbles_rec_away = 0;
				...
				$fumble_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=player_id';
				$jsonfumblehome = file_get_contents($fumble_string_home);
				$objfumblehome = json_decode($jsonfumblehome,true);
				...
				foreach ($fumble_id_home as $fumble => $fumblestat) {
					...
					$objplayerhome = json_decode($jsonplayerhome,true);
					$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
					$fumble_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&player_id=eq.'.$fumblestat.'&select=fumbles_lost,fumbles_rec,fumbles_rec_tds,fumbles_tot,fumbles_rec_yds,defense_frec,defense_frec_yds';
					$jsonfumblehome = file_get_contents($fumble_string_home);
					$objfumblehome = json_decode($jsonfumblehome,true);
					$fumbles_lost_home = 0;
					$fumbles_rec_home = 0;
				...
				$kicking_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=team,player_id,kicking_fga,kicking_fgm,kicking_xpa,kicking_xpmade,kicking_yds&order=team.asc,player_id.desc';
				$jsonkickingaway = file_get_contents($kicking_string_away);
				$objkickingaway = json_decode($jsonkickingaway,true);
				...
				foreach ($kicking_id_away as $kicking => $kickingstat) {
					...
					$objplayeraway = json_decode($jsonplayeraway,true);
					$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
					$kicking_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&player_id=eq.'.$kickingstat.'&select=team,player_id,kicking_fga,kicking_fgm,kicking_xpa,kicking_xpmade,kicking_yds&order=team.asc,player_id.desc';
					$jsonkickingaway = file_get_contents($kicking_string_away);
					$objkickingaway = json_decode($jsonkickingaway,true);
					$kicking_fga_away = 0;
					$kicking_fgm_away = 0;
				...
				$kicking_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=team,player_id,kicking_fga,kicking_fgm,kicking_xpa,kicking_xpmade,kicking_yds&order=team.asc,player_id.desc';
				$jsonkickinghome = file_get_contents($kicking_string_home);
				$objkickinghome = json_decode($jsonkickinghome,true);
				...
				foreach ($kicking_id_home as $kicking => $kickingstat) {
					...
					$objplayerhome = json_decode($jsonplayerhome,true);
					$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
					$kicking_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&player_id=eq.'.$kickingstat.'&select=team,player_id,kicking_fga,kicking_fgm,kicking_xpa,kicking_xpmade,kicking_yds&order=team.asc,player_id.desc';
					$jsonkickinghome = file_get_contents($kicking_string_home);
					$objkickinghome = json_decode($jsonkickinghome,true);
					$kicking_fga_home = 0;
					$kicking_fgm_home = 0;
				...
				$punting_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=team,player_id,punting_tot,punting_yds,punting_i20&order=team.asc,player_id.desc';
				$jsonpuntingaway = file_get_contents($punting_string_away);
				$objpuntingaway = json_decode($jsonpuntingaway,true);
				...
				foreach ($punting_id_away as $punting => $puntingstat) {
					...
					$objplayeraway = json_decode($jsonplayeraway,true);
					$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
					$punting_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&player_id=eq.'.$puntingstat.'&select=team,player_id,punting_tot,punting_yds,punting_i20&order=team.asc,player_id.desc';
					$jsonpuntingaway = file_get_contents($punting_string_away);
					$objpuntingaway = json_decode($jsonpuntingaway,true);
					$punting_tot_away = 0;
					$punting_yds_away = 0;
				...
				$punting_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=team,player_id,punting_tot,punting_yds,punting_i20&order=team.asc,player_id.desc';
				$jsonpuntinghome = file_get_contents($punting_string_home);
				$objpuntinghome = json_decode($jsonpuntinghome,true);
				...
				foreach ($punting_id_home as $punting => $puntingstat) {
					...
					$objplayerhome = json_decode($jsonplayerhome,true);
					$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
					$punting_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&player_id=eq.'.$puntingstat.'&select=team,player_id,punting_tot,punting_yds,punting_i20&order=team.asc,player_id.desc';
					$jsonpuntinghome = file_get_contents($punting_string_home);
					$objpuntinghome = json_decode($jsonpuntinghome,true);
					$punting_tot_home = 0;
					$punting_yds_home = 0;
				...
				$kickret_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=team,player_id,kickret_fair,kickret_oob,kickret_ret,kickret_tds,kickret_touchback,kickret_yds&order=team.asc,player_id.desc';
				$jsonkickretaway = file_get_contents($kickret_string_away);
				$objkickretaway = json_decode($jsonkickretaway,true);
				...
				foreach ($kickret_id_away as $kickret => $kickretstat) {
					...
					$objplayeraway = json_decode($jsonplayeraway,true);
					$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
					$kickret_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&player_id=eq.'.$kickretstat.'&select=team,player_id,kickret_fair,kickret_oob,kickret_ret,kickret_tds,kickret_touchback,kickret_yds&order=team.asc,player_id.desc';
					$jsonkickretaway = file_get_contents($kickret_string_away);
					$objkickretaway = json_decode($jsonkickretaway,true);
					$kickret_ret_away = 0;
					$kickret_tds_away = 0;
				...
				$kickret_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=team,player_id,kickret_fair,kickret_oob,kickret_ret,kickret_tds,kickret_touchback,kickret_yds&order=team.asc,player_id.desc';
				$jsonkickrethome = file_get_contents($kickret_string_home);
				$objkickrethome = json_decode($jsonkickrethome,true);
				...
				foreach ($kickret_id_home as $kickret => $kickretstat) {
					...
					$objplayerhome = json_decode($jsonplayerhome,true);
					$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
					$kickret_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&player_id=eq.'.$kickretstat.'&select=team,player_id,kickret_fair,kickret_oob,kickret_ret,kickret_tds,kickret_touchback,kickret_yds&order=team.asc,player_id.desc';
					$jsonkickrethome = file_get_contents($kickret_string_home);
					$objkickrethome = json_decode($jsonkickrethome,true);
					$kickret_ret_home = 0;
					$kickret_tds_home = 0;
				...
				$puntret_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=team,player_id,puntret_fair,puntret_oob,puntret_tot,puntret_tds,puntret_touchback,puntret_yds&order=team.asc,player_id.desc';
				$jsonpuntretaway = file_get_contents($puntret_string_away);
				$objpuntretaway = json_decode($jsonpuntretaway,true);
				...
				foreach ($puntret_id_away as $puntret => $puntretstat) {
					...
					$objplayeraway = json_decode($jsonplayeraway,true);
					$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
					$puntret_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&player_id=eq.'.$puntretstat.'&select=team,player_id,puntret_fair,puntret_oob,puntret_tot,puntret_tds,puntret_touchback,puntret_yds&order=team.asc,player_id.desc';
					$jsonpuntretaway = file_get_contents($puntret_string_away);
					$objpuntretaway = json_decode($jsonpuntretaway,true);
					$puntret_tot_away = 0;
					$puntret_tds_away = 0;
				...
				$puntret_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=team,player_id,puntret_fair,puntret_oob,puntret_tot,puntret_tds,puntret_touchback,puntret_yds&order=team.asc,player_id.desc';
				$jsonpuntrethome = file_get_contents($puntret_string_home);
				$objpuntrethome = json_decode($jsonpuntrethome,true);
				...
				foreach ($puntret_id_home as $puntret => $puntretstat) {
					...
					$objplayerhome = json_decode($jsonplayerhome,true);
					$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
					$puntret_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&player_id=eq.'.$puntretstat.'&select=team,player_id,puntret_fair,puntret_oob,puntret_tot,puntret_tds,puntret_touchback,puntret_yds&order=team.asc,player_id.desc';
					$jsonpuntrethome = file_get_contents($puntret_string_home);
					$objpuntrethome = json_decode($jsonpuntrethome,true);
					$puntret_tot_home = 0;
					$puntret_tds_home = 0;
				...
				$defense_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&select=team,player_id,defense_ast,defense_tkl,defense_sk,defense_int,defense_ffum&order=defense_tkl.desc,player_id.desc';
				$jsondefenseaway = file_get_contents($defense_string_away);
				$objdefenseaway = json_decode($jsondefenseaway,true);
				...
				foreach ($defense_id_away as $defense => $defensestat) {
					...
					$name = $objplayeraway[0]["first_name"][0].".".$objplayeraway[0]["last_name"];
					$defense_string_away = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$awaycode.'&player_id=eq.'.$defensestat.'&select=team,player_id,defense_ast,defense_tkl,defense_sk,defense_int,defense_ffum,defense_int_tds,defense_frec_tds&order=defense_tkl.desc,player_id.desc';
					$jsondefenseaway = file_get_contents($defense_string_away);
					$objdefenseaway = json_decode($jsondefenseaway,true);
					$defense_ast_away = 0;
					$defense_tkl_away = 0;
				...
				$defense_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&select=team,player_id,defense_ast,defense_tkl,defense_sk,defense_int,defense_int_tds,defense_ffum&order=defense_tkl.desc,player_id.desc';
				$jsondefensehome = file_get_contents($defense_string_home);
				$objdefensehome = json_decode($jsondefensehome,true);
				...
				foreach ($defense_id_home as $defense => $defensestat) {
					...
					$name = $objplayerhome[0]["first_name"][0].".".$objplayerhome[0]["last_name"];
					$defense_string_home = 'http://localhost:2096/play_player?gsis_id=eq.'.$pastgame.'&team=eq.'.$homecode.'&player_id=eq.'.$defensestat.'&select=team,player_id,defense_ast,defense_tkl,defense_sk,defense_int,defense_ffum,defense_int_tds,defense_frec_tds&order=defense_tkl.desc,player_id.desc';
					$jsondefensehome = file_get_contents($defense_string_home);
					$objdefensehome = json_decode($jsondefensehome,true);
					$defense_ast_home = 0;
					$defense_tkl_home = 0;
				...
				$front_summary_away_string = 'http://localhost:2096/play?gsis_id=eq.'.$pastgame.'&pos_team=eq.'.$awaycode.'&order=drive_id.asc,play_id.asc';
				$jsonfront_summary_away_string = file_get_contents($front_summary_away_string);
				$objfront_summary_away_string = json_decode($jsonfront_summary_away_string,true);
				...
				$front_summary_home_string = 'http://localhost:2096/play?gsis_id=eq.'.$pastgame.'&pos_team=eq.'.$homecode.'&order=drive_id.asc,play_id.asc';
				$jsonfront_summary_home = file_get_contents($front_summary_home_string);
				$objfront_summary_home = json_decode($jsonfront_summary_home,true);
		...
		$score_morris_area_chart = '<div class="tab-overflow overflow-right bg-black-darker" style="-webkit-border-radius: 0px;-moz-border-radius: 0px;border-radius: 0px;">
                                <ul class="nav nav-tabs nav-tabs-inverse" style="-webkit-border-radius: 0px;-moz-border-radius: 0px;border-radius: 0px;">
                                    <li class="active"><a href="#nav-tab2-1" data-toggle="tab">Past</a></li>
                                    <li class=""><a href="#nav-tab2-2" data-toggle="tab">Position</a></li>
                                </ul>
                            </div>
                            <div class="tab-content" style="padding:0px;">
                            	<div class="tab-pane fade active in" id="nav-tab2-1">
			        				<div class="widget-chart with-sidebar bg-black" style="-webkit-border-radius: 0px;-moz-border-radius: 0px;border-radius: 0px;margin-bottom: -35px;padding-bottom: 15px;">
			            				<div class="widget-chart-content">
			                				<h4 class="chart-title">
			                    			Last Matchup
			                    			<small>'.$date_p.' '.$lastyear.' meeting &nbsp; <a href="game.php?game_id='.$pastgame.'" style="color:#eee;">see full game <i class="fa fa-angle-double-right"></i></a></small>
			                				</h4>
			                				<div id="score-line-chart" class="morris-inverse" style="height: 260px;"></div>
			            				</div>
			        			...
			        			<div class="tab-pane fade" id="nav-tab2-2">
			        				<div class="widget-chart with-sidebar bg-black" style="-webkit-border-radius: 0px;-moz-border-radius: 0px;border-radius: 0px;margin-bottom: -35px;padding-bottom: 15px;">
			                			...
			                			<div id="position-bar-chart" class="morris-inverse" style="height: 260px;width:680px;"></div>
			            			</div>
			            			<div class="widget-chart-sidebar bg-black-darker">
			                			<div class="chart-number">
			                    			Position Ranks
			                    			<small>combined net yards</small>
			                			</div>
			            			</div>
			    				</div>
			    			</div>';
...
var handleMorrisBarChart = function() {
    ...
    Morris.Bar({
        element: 'position-bar-chart',
        data: [
            {x: 'Passing', y: ".$total_agg_passing_yds_away.", z: ".$total_agg_passing_yds_home."},
            {x: 'Rushing', y: ".$total_agg_rushing_yds_away.", z: ".$total_agg_rushing_yds_home."},
            {x: 'First Downs', y: ".$firstdowns_away_total.", z: ".$firstdowns_home_total."}
        ],
        xkey: 'x',
        ykeys: ['y', 'z'],
        parseTime: false,
...
DashboardV2.init();";
	...
	$widget[$i] = '<div class="panel panel-inverse game-'.$date.'" data-sortable-id="ui-widget-'.$sortid.'">
                        <div class="panel-heading">
                            ...
                            '.$progress_button.'
                            <h4 class="panel-title"><a href="game.php?game_id='.$date.'&type='.$season_type.'&year='.$year.'&week='.$week.'">'.$date_f.' '.$yearofgame.' - '.$awaycode.' vs. '.$homecode.'</a></h4>
                        </div>
...
$w = 0;
foreach ($globalleader_passing_yds as $key => $val) {
	$info = explode(" ", $key);
	$name = $info[0];
	$gsid = $info[1];
	$player_id = $info[2];
	$passing_yds = $val;
	$game = array_search($gsid, $globalgameref);
	if (!$pflag) { $label = "<div class='info passing' style='width: 100%;padding-top: 10px;padding-left: 10px;background-color:rgba(0, 0, 0, 0.55);'><small>PASSING</small></div>"; } else { $label = ""; }
	$line = $label.'<div class="info passing">
							<a href="player.php?player_id='.$player_id.'" style="color:#fff;">'.$name.'</a>
							<small><span style="color:#fff;">'.$passing_yds.'</span> YDs</small>
						</div>';
	$weeklyleaders[$w] = $line;
	$pflag = true;
	$w++;
}

foreach ($globalleader_rushing_yds as $key => $val) {
	$info = explode(" ", $key);
	$name = $info[0];
	$gsid = $info[1];
	$player_id = $info[2];
	$rushing_yds = $val;
	$game = array_search($gsid, $globalgameref);
	if (!$ruflag) { $label = "<div class='info rushing' style='width: 100%;padding-top: 10px;padding-left: 10px;background-color:rgba(0, 0, 0, 0.55);'><small>RUSHING</small></div>"; } else { $label = ""; }
	$line = $label.'<div class="info rushing">
							<a href="player.php?player_id='.$player_id.'" style="color:#fff;">'.$name.'</a>
							<small><span style="color:#fff;">'.$rushing_yds.'</span> YDs</small>
						</div>';
	$weeklyleaders[$w] = $line;
	$ruflag = true;
	$w++;
...
foreach ($globalleader_receiving_yds as $key => $val) {
	$info = explode(" ", $key);
	$name = $info[0];
	$gsid = $info[1];
	$player_id = $info[2];
	$receiving_yds = $val;
	$game = array_search($gsid, $globalgameref);
	if (!$reflag) { $label = "<div class='info receiving' style='width: 100%;padding-top: 10px;padding-left: 10px;background-color:rgba(0, 0, 0, 0.55);'><small>RECEIVING</small></div>"; } else { $label = ""; }
	$line = $label.'<div class="info receiving">
							<a href="player.php?player_id='.$player_id.'" style="color:#fff;">'.$name.'</a>
							<small><span style="color:#fff;">'.$receiving_yds.'</span> YDs</small>
						</div>';
	$weeklyleaders[$w] = $line;
	$reflag = true;
	$w++;
